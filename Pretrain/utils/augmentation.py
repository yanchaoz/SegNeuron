...
def create_identity_transformation(shape, subsample=1):
    ...
    axis_ranges = (
        np.arange(subsample_shape[d], dtype=np.float32) * step_width[d] for d in range(dims)
    )
...
def create_elastic_transformation(shape, control_point_spacing=100, jitter_sigma=10.0, subsample=1):
    ...
    control_points = tuple(
        max(1, int(round(float(shape[d]) / spacing[d]))) for d in range(len(shape))
    )
...
def random_offset(max_misalign):
    return Coordinate(
        (0,) + tuple(max_misalign - random.randint(0, 2 * int(max_misalign)) for d in range(2))
    )
...
def apply_transformation(image, transformation, interpolate=True, outside_value=0, output=None):
    ...
    return map_coordinates(
        image, transformation, output=output, order=order, mode="constant", cval=outside_value
    )
    ...
    def forward(self, inputs, mask=None):
        ...
        if skiprand < self.ratio:
            ...
            for k in range(inputs.shape[0]):
                inputs[k] = cv2.warpAffine(
                    inputs[k], rot_mat, inputs[k].shape, flags=cv2.INTER_LINEAR
                )
            if mask is not None:
                for k in range(mask.shape[0]):
                    mask[k] = cv2.warpAffine(
                        mask[k], rot_mat, mask[k].shape, flags=cv2.INTER_NEAREST
                    )
                return inputs, mask
            return inputs
        if mask is not None:
...
class ElasticAugment:
    ...
    def forward(self, imgs, mask):
        ...
        seg_transform = apply_transformation(
            mask,
            transform,
            interpolate=False,
...
def elastic_deform_3d_cuda(
    ...
    if padding is not None:
        image_in = np.pad(
            image_in, ((0, 0), (padding, padding), (padding, padding)), mode="reflect"
        )
        label_in = np.pad(
            label_in, ((0, 0), (padding, padding), (padding, padding)), mode="reflect"
        )
    ...
    dx = (
        gaussian_filter(
            (random_state.rand(*shape) * 2 - 1), sigma, order=0, mode="constant", cval=0
        )
        * alpha
    )
    dy = (
    ...
    gx = 2.0 * def_grid_x / (shape[2] - 1) - 1.0
    gy = 2.0 * def_grid_y / (shape[1] - 1) - 1.0
    gz = 2.0 * def_grid_z / (shape[0] - 1) - 1.0
    ...
    torch_im = torch.Tensor(np.expand_dims(np.expand_dims(image_in, axis=0), axis=0).copy()).to(
        device
    )
    torch_lb = torch.Tensor(np.expand_dims(label_in, axis=0).copy()).to(device)
    with torch.no_grad():
        torch_im_out = torch.nn.functional.grid_sample(
            torch_im, torch_grid, mode="bilinear", padding_mode="zeros"
        )
        torch_lb_out = torch.nn.functional.grid_sample(
            torch_lb, torch_grid, mode="bilinear", padding_mode="zeros"
        )
