...
def training_show(
    iters, inputs, label, pred_bound, cache_path, if_skele=None, skele=None, pred_skele=None
):
...
def class_color(lb):
    ...
    lb_color2[ids_1] = 255
    lb_color3[ids_1] = 0
    lb_color = np.concatenate(
        [
            lb_color1[:, :, :, np.newaxis],
            lb_color2[:, :, :, np.newaxis],
            lb_color3[:, :, :, np.newaxis],
        ],
        axis=3,
    )
    return lb_color
