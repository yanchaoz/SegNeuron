...
class SegCVTransformRandomCropRotateScale:
    """
    ...
    def transform_pair(self, sample0, sample1):
        ...
        if "xf_cv" in sample0:
            xf01 = affine.cat_nx2x3(
                local_xfs, np.stack([sample0["xf_cv"], sample1["xf_cv"]], axis=0)
            )
...
class SegCVTransformRandomFlip:
    ...
    def transform_pair(self, sample0, sample1):
        ...
        if "xf_cv" in sample0:
            ...
            flip_xlat_xy = flip_flags_xyd[:, :2] * (
                np.array(
                    [sample0["image_arr"].shape[:2][::-1], sample1["image_arr"].shape[:2][::-1]]
                ).astype(float)
                - 1
            )
