...
class Train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # randint includes both endpoints; randrange prevents the intermittent
        # legacy IndexError caused by selecting len(self.dataset).
        ...
        to_patch = Rearrange(
            "b c (f pf) (h p1) (w p2) -> b (f h w) (p1 p2 pf c)", p1=x_y_bx, p2=x_y_bx, pf=z_bx
        )
        to_img = Rearrange(
        ...
        random_dimensions = np.random.choice(
            int(20 / z_bx) * int(128 / x_y_bx) * int(128 / x_y_bx),
            size=int(int(20 / z_bx) * int(128 / x_y_bx) * int(128 / x_y_bx) * mask_ratio),
            replace=False,
        )
