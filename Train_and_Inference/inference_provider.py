...
class Provider_valid(Dataset):
    def __init__(self, cfg, valid_data=None):
        ...
        for k in range(len(self.labels)):
            ...
            self.gt_affs.append(
                seg_to_affgraph(temp, mknhood3d(1), pad="replicate").astype(np.float32)
            )

        self.num_zyx = [
            (self.origin_data_shape[0] - self.out_size[0]) // self.stride[0] + 2,
            (self.origin_data_shape[1] - self.out_size[1]) // self.stride[1] + 2,
            (self.origin_data_shape[2] - self.out_size[2]) // self.stride[2] + 2,
        ]
        self.valid_padding = [
            ((self.num_zyx[0] - 1) * self.stride[0] + self.out_size[0] - self.origin_data_shape[0])
            // 2,
            ((self.num_zyx[1] - 1) * self.stride[1] + self.out_size[1] - self.origin_data_shape[1])
            // 2,
            ((self.num_zyx[2] - 1) * self.stride[2] + self.out_size[2] - self.origin_data_shape[2])
            // 2,
        ]

        for k in range(len(self.dataset)):
            self.dataset[k] = np.pad(
                self.dataset[k],
                (
    ...
    def add_vol(self, affs_vol):
        ...
        self.out_affs[
            :,
            fromz : fromz + self.out_size[0],
            fromx : fromx + self.out_size[1],
            fromy : fromy + self.out_size[2],
